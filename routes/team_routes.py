...
def add_hypermedia_links(team, members=False):
    ...
    team_with_links["_links"] = {
        ...
        "members": {"href": url_for("team_routes.get_team_members", team_id=team_id, _external=True)},
...
def add_member_hypermedia_links(team_id, member):
    ...
    member_with_links["_links"] = {
        ...
        "update": {"href": url_for("team_routes.update_team_member", team_id=team_id, user_id=user_id, _external=True)},
        "delete": {"href": url_for("team_routes.remove_team_member", team_id=team_id, user_id=user_id, _external=True)},
        "collection": {"href": url_for("team_routes.get_team_members", team_id=team_id, _external=True)}
    }
...
def add_team_member(team_id):
    ...
    if status_code == 201 and isinstance(result, dict) and "user_id" in data:
        ...
        result["_links"] = {
            ...
            "members": {"href": url_for("team_routes.get_team_members", team_id=team_id_str, _external=True)},
            "member": {"href": url_for("team_routes.update_team_member", team_id=team_id_str, user_id=user_id_str, _external=True)}
        }
...
def update_team_member(team_id, user_id):
    ...
    if status_code == 200 and isinstance(result, dict):
        ...
        result["_links"] = {
            "team": {"href": url_for("team_routes.get_team", team_id=team_id_str, _external=True)},
            "members": {"href": url_for("team_routes.get_team_members", team_id=team_id_str, _external=True)},
            "delete": {"href": url_for("team_routes.remove_team_member", team_id=team_id_str, user_id=user_id_str, _external=True)}
        }
...
def remove_team_member(team_id, user_id):
    ...
    if status_code == 200 and isinstance(result, dict):
        result["_links"] = {
            "team": {"href": url_for("team_routes.get_team", team_id=team_id_str, _external=True)},
            "members": {"href": url_for("team_routes.get_team_members", team_id=team_id_str, _external=True)},
            "add_member": {"href": url_for("team_routes.add_team_member", team_id=team_id_str, _external=True)}
        }
...
def get_team_members(team_id):
    ...
    if status_code == 200 and isinstance(result, dict):
        ...
        if "members" in result and isinstance(result["members"], list):
            result["members"] = [add_member_hypermedia_links(team_id_str, member) for member in result["members"]]
