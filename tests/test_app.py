    ...
    app.config.update(
        {
            ...
            "SQLALCHEMY_DATABASE_URI": "postgresql://admin:changeme@localhost/task_management_db",  # Test database URI
            "SQLALCHEMY_TRACK_MODIFICATIONS": False,  # Disables modification tracking for performance
            "JWT_SECRET_KEY": "change-me-jwt",  # Secret key for JWT encoding/decoding
...
def app_with_test_config():
    ...
    app.config.update({
        "TESTING": True,
        "SQLALCHEMY_DATABASE_URI": "postgresql://admin:changeme@localhost/task_management_db",
...
def test_create_app_with_test_config(app_with_test_config):
    ...
    assert app_with_test_config.config["SQLALCHEMY_DATABASE_URI"] == "postgresql://admin:changeme@localhost/task_management_db"
