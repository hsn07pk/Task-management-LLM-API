...
def test_run_command_failure():
    ...
    with patch('subprocess.run') as mock_run:
        ...
        with patch('code_quality.print_header') as mock_print_header:
            ...
            mock_run.assert_called_once_with("invalid command", shell=True, capture_output=True, text=True)
            assert result == 1
                ...
                with patch('sys.stdout', new=StringIO()) as mock_stdout:
                    with patch('code_quality.run_command', return_value=0) as mock_run_command:
                        ...
                        with patch('code_quality.datetime', MockDateTime):
                            ...
                            mock_file.assert_called_with("reports/code_quality_report_20230101_120000.txt", "w")
                ...
                with patch('sys.stdout', new=StringIO()):
                    with patch('code_quality.run_command', return_value=0) as mock_run_command:
                        ...
                        with patch('code_quality.datetime', MockDateTime):
                            ...
                            mock_run_command.assert_any_call("black --check .", "BLACK - Code formatting check")
                            mock_run_command.assert_any_call("isort --check-only --profile black .", "ISORT - Import order check")
                            mock_run_command.assert_any_call("flake8 . --count --select=E9,F63,F7,F82 --show-source --statistics", "FLAKE8 (Fatal Errors)")
                            mock_run_command.assert_any_call("flake8 . --count --exit-zero --max-complexity=10 --max-line-length=127 --statistics", "FLAKE8 (Style Warnings)")
                            mock_run_command.assert_any_call("vulture .", "VULTURE - Dead code detection")
                            mock_run_command.assert_any_call("bandit -r . -c pyproject.toml", "BANDIT - Security issues")
                            mock_run_command.assert_any_call("pytest --cov=. tests/", "PYTEST - Running tests with coverage") 